...
const addMedia = asyncHandler(async (req, res) => {
  ...
  const acknowledgment = await Acknowledgment.findById(id);
  if (acknowledgment) {
    // Wassenger File upload
    // let { data: mediaUpload } = await axios.post(
    //   wassengerAPI_URL + "/files",
    //   {
    ...
    //   return res.json({
    //     success: false,
    //     error: "Unable to upload to wassenger",
    //   });
    // }
    ...
    //   return res.json({
    //     success: false,
    //     error: mediaUpload.message || "Unable to upload to wassenger",
    //   });
    // }
    acknowledgment.media.push({
      filename: req.file.filename,
      selected: true,
      originalFilename: req.file.originalname,
      //   wassengerFileId: mediaUpload.id,
    });
    await acknowledgment.save();
    res.json({
      success: true,
      file: acknowledgment.media[acknowledgment.media.length - 1],
    });
    return;
  }
  res.json({ success: false, error: "Database error" });
...
const deleteSingleMedia = asyncHandler(async (req, res) => {
  ...
  const acknowledgment = await Acknowledgment.findById(docId);
  if (acknowledgment) {
    const { filename /*, wassengerFileId*/ } = acknowledgment.media.find(
      (e) => e._id.toString() === mediaId
    );
    // let { data: wassengerResponse } = await axios.delete(
    //   wassengerAPI_URL + "/files/" + wassengerFileId,
    //   {
    //     headers: {
    //       "Content-Type": "application/json",
    //       Authorization: env.WASSENGER_TOKEN,
    ...
    //   }
    // );
    let media = acknowledgment.media.filter(
      (e) => e._id.toString() !== mediaId
    );
